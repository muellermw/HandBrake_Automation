#!/bin/sh
############################################################################################################################################
# DESCRIPTION:     Video compression script that uses HandBrakeCLI for encoding: this version takes every .mkv file in the                 #
#                  compressDirectory and compresses them. When completed, the script moves the compressed and uncompressed files           #
#                  into finished and backup directories.                                                                                   #
...
finishedDirectory=/home/max/Max-Server-Files/odin\ server/Finished_Compressing
  backupDirectory=/home/max/Max-Server-Files/odin\ server/Backup_Raw_Movie_Files

  logFile="$compressDirectory/CompressionLog.txt"
errorFile="$compressDirectory/HandBrakeScriptErrorReport.txt"
...
mkvFile=$(find "$compressDirectory" -name "*$mkvFileExt" | head -n 1)
...
if [ ! -d "$finishedDirectory" ]; then
    mkdir "$finishedDirectory"
fi

mv "$compressDirectory/$compressedVideoFile" "$finishedDirectory/"
if [ $? -ne 0 ]; then
    echo "Could not move $compressedVideoFile to $finishedDirectory. Did HandBrakeCLI error out as well?" >> "$errorFile"
else
    echo "$compressedVideoFile is now located in $finishedDirectory"
fi
